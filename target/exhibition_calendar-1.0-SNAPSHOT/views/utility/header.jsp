   ...
   <body>
      ...
      <c:choose>
         <c:when test="${role == 'MODERATOR' || role == 'ADMIN'}">
            <a  href="${pageContext.request.contextPath}/controller?command=moderatorHome">
               <fmt:message key="header.moderator"/>
            </a>
         </c:when>
      </c:choose>
      <c:choose>
      ...
      <c:choose>
         <c:when test="${role == 'USER' || role == 'MODERATOR' || role == 'ADMIN'}">
            ...
            &nbsp;&nbsp;
            <a  href="${pageContext.request.contextPath}/controller?command=userHome">
               <fmt:message key="header.userInfo"/>
            </a>
            <br>
         </c:when>
         <c:otherwise>
